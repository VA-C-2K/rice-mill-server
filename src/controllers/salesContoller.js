...
export const fetchSales = asyncHandler(async (req, res) => {
    const { sale_id, page, perPage = 5, sort: { next_due_on = -1, date = -1 } } = req.query;
    try {
        if (sale_id) {
...
export const createSales = asyncHandler(async (req, res) => {
    const { date, total_amount, discount, final_amount_paid, remainig_amount, next_due_on, quantity, vehicle_number, vehicle_details, product_details, customer_details } = req.body;
    ...
    try {
        const newSale = new Sales({ date, total_amount, discount, final_amount_paid, remainig_amount, next_due_on, quantity, vehicle_number, vehicle_details, product_details, customer_details, created_by: user, modified_by: user });
